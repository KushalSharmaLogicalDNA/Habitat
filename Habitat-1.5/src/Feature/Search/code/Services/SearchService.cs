...
namespace Sitecore.Feature.Search.Services
{
    ...
    public class SearchService
    {
        ...
        public SearchResultsViewModel SearchFromTopResultsSolr(string query, int count)
        {
            var searchQuery = new SearchQuery
            {
                QueryText = query,
            ...
            var results = this.SearchServiceRepository.Get(this.SearchContextRepository.Get()).SearchSolr(searchQuery);
            return this.SearchResultsViewModelFactory.Create(searchQuery, results, 1, count);
